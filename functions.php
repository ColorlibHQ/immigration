 ...
 */
if ( ! function_exists( 'immigration_get_page_by_title' ) ) {
	function immigration_get_page_by_title( $title, $output = OBJECT, $post_type = 'page' ) {
		$query = new WP_Query(
			array(
				'post_type'              => $post_type,
				'title'                  => $title,
 ...
 */
if ( ! function_exists( 'immigration_modern_supports' ) ) {
	function immigration_modern_supports() {
		add_theme_support( 'responsive-embeds' );
		add_theme_support( 'align-wide' );
		add_theme_support( 'editor-styles' );
	}
	add_action( 'after_setup_theme', 'immigration_modern_supports', 20 );
}
